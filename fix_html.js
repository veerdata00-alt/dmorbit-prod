...
const pacingEnd = html.indexOf('<!-- Right column: Recent Activity Feed (Live Feed) -->');
if (pacingStart !== -1 && pacingEnd !== -1) {
    html = html.replace('grid-template-columns: 1fr 1fr; gap: 24px;" class="overview-details-grid grid grid-cols-1 lg:grid-cols-2"', 
                        'grid-template-columns: 1fr; gap: 24px;" class="overview-details-grid grid grid-cols-1"');
